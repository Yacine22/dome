...
class FenetrePrincipale:
	...
	def __init__(self):
		...
		self.boutonCapturePhoto = Button(self.frame, text="Turntable",foreground=_from_rgb((176,175,179)),relief="flat",background=_from_rgb((41,40,46)),image=self.photoTurntable,compound=TOP,cursor="tcross",command=self.ouvrirCapturePhoto)
...
class FenetreCapturePhoto:
	...
	def startCapture(self):
		...
		#Camera
		self.camera = picamera.PiCamera()
		self.camera.resolution = (480, 380) #3280 × 2464 #1640, 1232
		self.camera.iso=100
		self.camera.rotation = 180
		self.camera.preview_fullscreen = False
		self.camera.still_stats = 'true' #MODIFICATION190621
